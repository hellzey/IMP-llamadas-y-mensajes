...
$query = "
    SELECT u.id_usuario, u.nombre, u.foto_perfil, u.ultima_actividad
    FROM usuarios u
...
<body id="main-body"> 
    ...
    <div class="container" id="main-container">
        ...
        <?php while ($row = $result->fetch_assoc()) { 
            $ultima_actividad = strtotime($row['ultima_actividad']);
            $ahora = time();
            $en_linea = ($ahora - $ultima_actividad <= 120);
        ?>
            <li data-id="<?php echo $row['id_usuario']; ?>">
                <img src="data:image/jpeg;base64,<?php echo base64_encode($row['foto_perfil']); ?>" alt="Amigo" class="friend-image">
                <?php echo htmlspecialchars($row['nombre']); ?> - 
                <span class="estado-usuario <?php echo $en_linea ? 'online' : 'offline'; ?>">
                    <?php echo $en_linea ? 'En línea' : 'Offline'; ?>
                </span>
            </li>
        <?php } ?>
        </ul>
